...
from sklearn.model_selection import GroupKFold, StratifiedKFold, StratifiedShuffleSplit, train_test_split
from sklearn.metrics import cohen_kappa_score, accuracy_score, balanced_accuracy_score


# Get win markets where preoff volume > 100k
def classify_odds_movements(odds_pct_change):
    if odds_pct_change < -0.005:
        return 'short'
    if odds_pct_change > 0.005:
        return 'drift'
    else:
...
def refactor_data(df, off_time, resample_freq=None, n_lags=5):
    ...
    for n in range(1, n_lags):
        ...
        df[f'odds_change_lag_{n}'] = df['odds_pct_change'].shift(n)
